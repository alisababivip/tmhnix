...
import { useAiTradingInvestmentStore } from '~~/store/extensions/ai-trading/admin/investment'
import { useAiTradingPlanStore } from '~~/store/extensions/ai-trading/admin/plans'
definePageMeta({
...
const items = computed(() =>
  aiTradingInvestmentStore.investments
    .filter((investment) => {
...
const view = (uuid: string) => {
  router.push(`/admin/extensions/ai-trading/investments/${uuid}`)
}
        ...
        >
          ...
          >
            ...
            <BaseListbox
              ...
              :items="aiTradingPlanStore.plans.map((plan) => plan.title)"
            ...
            >
              ...
              >
                ...
                <template #end>
                  ...
                  >
                    <BaseTag
                      :color="resultStatus(item.result ?? 'PEDING')"
                      flavor="pastel"
                      condensed
                      >{{ resultSign(item.result)
                      }}{{
                        item.result === 'DRAW' ? 0 : item.profit ?? 'Pending'
                      }}</BaseTag
                    >
                  </TableFlexTableCell>
                  <TableFlexTableCell
