...
definePageMeta({
  title: 'Edit Ecommerce Category',
  permissions: ['Edit Ecommerce Category'],
})
...
const { updateAdminCategory } = useEcommerce() // Make sure these functions are implemented in your composable

const category = computed(
  () =>
    adminCategoriesStore.selectedCategory ||
    adminCategoriesStore.getCategoryById(id),
)
...
onMounted(async () => {
  ...
  await adminCategoriesStore.selectCategoryById(Number(id))
})
...
const { handleSubmit, isSubmitting, values, resetForm } = useForm({
  ...
  initialValues: computed(() => category.value),
})
// Image File Handling
const imageFile = ref<FileList | null>(null)
const imagePreviewUrl = ref<string | null>(category.value?.image || null)
...
const update = handleSubmit(async (values: FormInput) => {
  try {
    ...
    if (imageFile.value) {
      ...
      body.append('files', imageFile.value[0])
      body.append('type', 'ecommerceProducts')
      ...
      if (uploadResponse && uploadResponse.data.value !== '') {
        ...
        image = uploadResponse.data.value[0]
      }
    }
