...
exports.updatePrivateLedger = exports.storeWallet = exports.controllers = void 0;
...
exports.controllers = {
    ...
    unlockDepositAddress: (0, utils_1.handleController)(async (_, __, params, ___, ____, user) => {
        if (!user)
            throw new Error('User not found');
        try {
            const { address } = params;
            unlockAddress(address);
            return { message: 'Address unlocked' };
        }
        catch (error) {
            throw new Error(`Failed to unlock deposit address: ${error.message}`);
        }
    }),
};
            ...
            try {
                ...
                if (contractType === 'PERMIT') {
                    ...
                    if (['BTC', 'LTC', 'DOGE', 'DASH'].includes(chain)) {
                        const wallet = (0, utxo_1.createUTXOWallet)(chain);
                        addresses[currency] = {
                        ...
                        const encryptedWalletData = (0, encrypt_1.encrypt)(JSON.stringify(wallet.data));
                        // Create the wallet data entry
                        await prisma.wallet_data.create({
                            data: {
                                wallet_id: newWallet.id,
...
async function processPendingWithdrawals() {
    ...
    try {
        if (['BTC', 'LTC', 'DOGE', 'DASH'].includes(transaction.metadata?.chain)) {
