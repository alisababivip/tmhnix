...
exports.listBinaryReferrals = exports.listUnilevelReferrals = exports.listDirectReferrals = void 0;
...
async function listDirectReferrals(user) {
    ...
    const referrals = await prisma_1.default.mlm_referral.findMany({
        where: { referrerUuid },
        include: {
            referred: {
                select: {
    ...
    const rootUserRewardsCount = referrals[0]?.referrer?.referral_rewards.length || 0;
    return {
        uuid: user.uuid,
...
async function listUnilevelReferrals(user, mlmSettings) {
    ...
    const rootNode = await prisma_1.default.mlm_unilevel_node.findFirst({
    ...
    async function fetchUnilevelDownlines(nodeIds, level = 2) {
        ...
        const nodes = await prisma_1.default.mlm_unilevel_node.findMany({
            where: { id: { in: nodeIds } },
            include: {
                referral: {
                    include: {
        ...
        for (const node of nodes) {
            ...
            const childDownlines = await fetchUnilevelDownlines(childNodeIds, level + 1);
            allDownlines.push({
    ...
    const rootUserRewardsCount = await prisma_1.default.mlm_referral_reward.count({
        where: { referrerUuid },
    });
    return {
        uuid: user.uuid,
...
async function listBinaryReferrals(user, mlmSettings) {
    ...
    const rootNodes = await prisma_1.default.mlm_binary_node.findMany({
        where: { referral: { referrerUuid } },
        select: { id: true },
    });
    if (!rootNodes.length) {
    ...
    async function fetchBinaryDownlines(nodeIds, level = 2) {
        ...
        const nodes = await prisma_1.default.mlm_binary_node.findMany({
            where: { id: { in: nodeIds } },
            include: {
                referral: {
                    include: {
        ...
        const referredUsersDetails = await prisma_1.default.user.findMany({
        ...
        const userDetailsMap = new Map(referredUsersDetails.map((user) => [user.uuid, user]));
        const downlines = [];
        for (const node of nodes) {
            const { referral } = node;
            ...
            downlines.push({
                ...
                referredCount: leftDownline.length + rightDownline.length + referredDownlines.length,
                downlines: [...leftDownline, ...rightDownline, ...referredDownlines],
            });
        }
        return downlines.length > 0 ? downlines : [];
    }
    const allDownlines = await fetchBinaryDownlines(rootNodes.map((node) => node.id), 2);
    const rootUserRewardsCount = await prisma_1.default.mlm_referral_reward.count({
        where: { referrerUuid },
    });
    return {
