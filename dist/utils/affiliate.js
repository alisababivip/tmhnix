...
exports.handleReferralRegister = exports.processRewards = exports.getMlmSettings = void 0;
/* eslint-disable prettier-vue/prettier */
const logger_1 = require("~~/logger");
const prisma_1 = __importDefault(require("./prisma"));
const logger = (0, logger_1.createLogger)('MLM Affiliate');
async function getMlmSettings() {
...
exports.getMlmSettings = getMlmSettings;
async function processRewards(userId, amount, conditionName, currency, settingsObject = null) {
    if (!settingsObject) {
    ...
    const condition = await prisma_1.default.mlm_referral_condition.findFirst({
...
async function processDirectRewards(condition, referredUuid, amount) {
    ...
    const referral = await prisma_1.default.mlm_referral.findFirst({
        where: { referredUuid },
    });
    if (!referral) {
        return;
    ...
    const hasReceivedReward = (await prisma_1.default.mlm_referral_reward.count({
        where: {
    ...
    await prisma_1.default.mlm_referral_reward.create({
        data: {
            referrerUuid: referral.referrerUuid,
            condition_id: condition.id,
            reward: rewardAmount,
...
async function findUplines(userId, systemType, levels) {
    ...
    for (let i = 0; i < levels; i++) {
        ...
        if (systemType === 'BINARY') {
            referral = await prisma_1.default.mlm_binary_node.findFirst({
                where: { referral: { referredUuid: currentUserId } },
                include: { referral: { include: { referrer: true } } },
            });
        }
        else {
            referral = await prisma_1.default.mlm_unilevel_node.findFirst({
                where: { referral: { referredUuid: currentUserId } },
                include: { referral: { include: { referrer: true } } },
            });
        }
        if (!referral || !referral.referral.referrerUuid)
            break;
...
async function processBinaryRewards(condition, userId, depositAmount, mlmSettings) {
    ...
    for (let i = uplines.length - 1; i >= 0; i--) {
        ...
        const levelRewardPercentage = mlmSettings.binary.levels_percentage.find((l) => l.level === levelIndex)?.value;
...
async function processUnilevelRewards(condition, userId, depositAmount, mlmSettings) {
    ...
    for (let i = uplines.length - 1; i >= 0; i--) {
        ...
        const levelRewardPercentage = mlmSettings.unilevel.levels_percentage.find((l) => l.level === levelIndex)?.value;
        if (levelRewardPercentage === undefined) {
            continue;
        }
        const rewardAmount = depositAmount * (levelRewardPercentage / 100);
...
const handleReferralRegister = async (refUuid, userUuid) => {
    const referrer = await prisma_1.default.user.findUnique({
        where: { uuid: refUuid },
    });
    if (referrer) {
        const referral = await prisma_1.default.mlm_referral.create({
            data: {
        ...
        const mlmSystem = (await prisma_1.default.settings.findFirst({ where: { key: 'mlm_system' } })).value;
        if (mlmSystem === 'BINARY') {
            await handleBinaryMlmReferralRegister(referral.referrerUuid, referral.id);
        }
        else if (mlmSystem === 'UNILEVEL') {
            await handleUnilevelMlmReferralRegister(referral.referrerUuid, referral.id);
        }
    }
};
...
const handleBinaryMlmReferralRegister = async (referrerUuid, referralId) => {
    ...
    const referrerNode = await prisma_1.default.mlm_binary_node.findFirst({
        where: { referral: { referredUuid: referrerUuid } },
    });
    const placement = referrerNode && referrerNode.left_child_id ? 'right' : 'left';
    // Create a new binary node for the new user
    await prisma_1.default.mlm_binary_node.create({
        data: {
...
const handleUnilevelMlmReferralRegister = async (referrerUuid, referralId) => {
    ...
    await prisma_1.default.mlm_unilevel_node.create({
        data: {
            referral_id: referralId,
